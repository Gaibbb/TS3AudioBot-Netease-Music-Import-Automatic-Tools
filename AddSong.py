...
def get_song_list_kg(play_list_id, headers):
    ...
    for i in range(math.ceil(count / 300)):
        i += 1
        url = "http://121.37.225.70:4001/playlist/track/all?page=" + str(i) + "&pagesize=300&id=" + play_list_id

        response = requests.get(url, headers=headers)
        data = json.loads(response.text)
        data = data['data']['info']
        song_list_len_src = len(data)
        song_count += song_list_len_src
    ...
    return song_list_name, song_list_hash, song_count
...
def add_song_kg(addr, port, song_list, song_list_len, listId, song_begin_number, headers):
    ...
    for i in range(song_list_len):
        ...
        addurl = "http://" + addr + ":" + port + "/api/bot/use/0/(/list/add/" + str(listId) + "/" + str(songurl_trans) + ")"
